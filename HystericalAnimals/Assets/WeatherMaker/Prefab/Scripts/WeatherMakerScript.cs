...
namespace DigitalRuby.WeatherMaker
{
    ...
    public class WeatherMakerScript : MonoBehaviour
    {
        ...
        [Tooltip("Main camera to determine orthographic settings and other render settings.")]
        public Camera Camera;

        [Tooltip("Cameras to not render Weather Maker in. All other cameras, including temporary water cameras or 360 video cameras will get rendered with Weather Maker unless in this list.")]
        public System.Collections.Generic.List<Camera> IgnoreCameras;

        [Tooltip("Whether to ignore reflection probes for Weather Maker command buffers. For an indoor scene or if you don't care to reflect clouds, fog, etc. set this to true.")]
        ...
        [Header("Clouds")]
        [Tooltip("Cloud type. This is a shortcut to set various properties on the current cloud profile. In 2D, clouds are either none or any other option will be storm. Set to custom to use your own profile.")]
        ...
        public WeatherMakerDayNightCycleManagerScript DayNightScript;
        ...
        [Tooltip("Snow overlay script, null if none")]
        public WeatherMakerFullScreenOverlayScript SnowOverlayScript;
        ...
        /// <summary>
        /// Allows adding additional intensity modifiers by key
        /// </summary>
        [NonSerialized]
        public readonly System.Collections.Generic.Dictionary<string, float> IntensityModifierDictionary = new System.Collections.Generic.Dictionary<string, float>(StringComparer.OrdinalIgnoreCase);
        ...
        public static Color EvaluateSunGradient(Gradient gradient)
        {
            if (gradient == null)
            {
                return Color.white;
            }
            float sunGradientLookup = GetSunGradientLookup(Instance.Camera, Instance.Sun.Transform);
            return gradient.Evaluate(sunGradientLookup);
        }
        ...
        public static Color EvaluateGradient(Gradient gradient, float lookup)
        {
            ...
            return gradient.Evaluate(lookup);
        }

        public static float GetSunGradientLookup(Camera camera, Transform sun)
        {
            float sunGradientLookup = (camera != null && camera.orthographic ? sun.forward.z : -sun.forward.y);
            sunGradientLookup = ((sunGradientLookup + 1.0f) * 0.5f);
            return sunGradientLookup;
        }

        public void GetSnowIntensity(WeatherMakerOutputParameter<float> param)
        {
            param.Value = SnowScript.Intensity;
        }
        ...
        public static bool AssertExists()
        {
            ...
            {
                Debug.LogError("WeatherMakerPrefab missing from scene (WeatherMakerScript and WeatherMakerDayNightCycleManagerScript are required and must not be deleted).");
                return false;
            }
        ...
        private void SetGlobalShaders()
        {
            if (Sun != null)
            {
                ...
                sunColor = new Vector4(Sun.TintColor.r, Sun.TintColor.g, Sun.TintColor.b, Sun.TintColor.a * Sun.TintIntensity);
        ...
        private void UpdateWeatherProfile()
        {
            ...
            if (CloudScript != null && _WeatherProfile.CloudType == WeatherMakerCloudType.Custom)
            {
                CloudScript.CloudProfile = _WeatherProfile.CloudProfile;
            }
            if (FogScript != null && FogScript.FogProfile != null)
            {
                FogScript.TransitionFogDensity(FogScript.FogProfile.FogDensity, _WeatherProfile.FogDensity, CloudChangeDuration);
                FogScript.FogProfile.FogHeight = _WeatherProfile.FogHeight;
                FogScript.FogProfile.FogMode = _WeatherProfile.FogMode;
            }
            if (WindScript != null)
            {
        ...
        {
            if (IgnoreCameras.Contains(camera) || (IgnoreReflectionProbes && camera.CachedName().IndexOf("probe", StringComparison.OrdinalIgnoreCase) >= 0))
            {
                return;
            }
        ...
        private void CameraPreRenderWrapper(Camera camera)
        {
            if (IgnoreCameras.Contains(camera) || (IgnoreReflectionProbes && camera.CachedName().IndexOf("probe", StringComparison.OrdinalIgnoreCase) >= 0))
            {
                return;
        ...
        private void CameraPreCull(Camera camera)
        {
            ...
            {
                LightManagerScript.PreCullCamera(camera);
            }
            ...
            if (CloudScript != null && CloudScript.isActiveAndEnabled)
            {
                CloudScript.PreCullCamera(camera);
            }
            if (SnowOverlayScript != null && SnowOverlayScript.isActiveAndEnabled)
            {
                SnowOverlayScript.PreCullCamera(camera);
            }
            if (FogScript != null && FogScript.isActiveAndEnabled)
            {
        ...
        private void CameraPreRender(Camera camera)
        {
            ...
            if (LightManagerScript != null && LightManagerScript.isActiveAndEnabled)
            {
                LightManagerScript.PreRenderCamera(camera);
            }
            if (FogScript != null && FogScript.isActiveAndEnabled)
            {
                FogScript.PreRenderCamera(camera);
            }
            if (CommandBufferManagerScript != null && CommandBufferManagerScript.isActiveAndEnabled)
            {
                CommandBufferManagerScript.PreRenderCamera(camera);
            }
        }
        ...
        {
            UpdateSoundsVolumes();
            if (PrecipitationScript != null)
            {
                PrecipitationScript.ExternalIntensityMultiplier = 1.0f;
                foreach (float multiplier in IntensityModifierDictionary.Values)
                {
                    PrecipitationScript.ExternalIntensityMultiplier *= multiplier;
                }
            }
            if (LightningScript != null)
            {
                LightningScript.ExternalIntensityMultiplier = 1.0f;
                foreach (float multiplier in IntensityModifierDictionary.Values)
                {
                    LightningScript.ExternalIntensityMultiplier *= multiplier;
                }
            }
            if (WindScript != null)
            {
                WindScript.ExternalIntensityMultiplier = 1.0f;
                foreach (float multiplier in IntensityModifierDictionary.Values)
                {
                    WindScript.ExternalIntensityMultiplier *= multiplier;
                }
            }
        }
