...
#if !defined(WEATHER_MAKER_FOG_EXPONENTIAL) && !defined(WEATHER_MAKER_FOG_LINEAR) && !defined(WEATHER_MAKER_FOG_EXPONENTIAL_SQUARED) && !defined(WEATHER_MAKER_FOG_CONSTANT)
...
//	}
	...
	for (float i = 0.0; i < _FogSunShaftsParam1.y; i++)
	{
	...
	return color * _FogSunShaftsParam1.w * _FogSunShaftsTintColor.rgb * lightViewportPos.w * _FogSunShaftsParam1.z * fogFactorReducer1 * fogFactorReducer2;
}
...
void ComputeDirectionalLightFog(float3 rayOrigin, float3 rayDir, float rayLength, float fogFactor, float depth01, out fixed3 lightColor, float2 screenUV)
{
	...
	fixed3 ambient = min(1.0, _WeatherMakerAmbientLightColor.rgb * _AmbientLightMultiplier * max(0.0, 1.0 - _WeatherMakerSunColor.a));
