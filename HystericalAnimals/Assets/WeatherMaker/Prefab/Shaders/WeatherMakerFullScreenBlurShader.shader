...
Shader "WeatherMaker/WeatherMakerFullScreenBlurShader"
{
	Properties
	{
		...
		_BlurDepthMin("Blur depth threshold", Range(0.0, 1.0)) = 0.00001
	...
	SubShader
	{
		...
		v2f vert (appdata_base v)
		{
			...
			if (_Blur7 == 1.0)
			{
				...
				o.offsets = float4(_MainTex_TexelSize.x * blurSize * 0.6667, _MainTex_TexelSize.y * blurSize * 0.6667, 0.0, 0.0);
			}
			else
			{
				o.offsets = float4
				(
					_MainTex_TexelSize.x * blurSize,
					_MainTex_TexelSize.x * -blurSize,
					_MainTex_TexelSize.y * blurSize,
					_MainTex_TexelSize.y * -blurSize
				);
			}
		...
		Pass
		{
			...
			inline fixed4 blurTap(float2 uv)
			{
			...
			fixed4 frag(v2f i) : SV_Target
			{
                ...
                float depth1 = (_BlurDepthMin < 0.00001 || _WeatherMakerVREnabled ? 1.0 : WM_SAMPLE_DEPTH_DOWNSAMPLED_01(i.uv));

				// debug depth
				//return fixed4(sourceDepth, sourceDepth, sourceDepth, 1.0);
				//return fixed4(depth1, depth1, depth1, 1.0);
				//return fixed4(abs(depth1 - sourceDepth), abs(depth1 - sourceDepth), abs(depth1 - sourceDepth), 1.0);

				fixed4 col1 = blurTap(i.uv);
				fixed col1Alpha = col1.a;

				// if pixel has no alpha, do not attempt to blur, 0 alpha pixels usually are a sign of being blocked by the depth buffer
				UNITY_BRANCH
				if (col1Alpha == 0.0 || sourceDepth < depth1 - 0.05)
				{
					return fixed4Zero;
				}
				else
