...
public class FormationManagerEditor : Editor
{
	...
	void OnSceneGUI ()
	{
		...
		if (_target.leaderAgent != null) {
            ...
            Handles.ArrowHandleCap (0, _target.leaderAgent.transform.position, _target.leaderAgent.transform.rotation, 4f, EventType.Repaint);
		}
		...
		case FormationManager.Formations.SemiCircle:
			...
			for (int i = 0; i < _target.agentsNum; i++) {
				...
				Handles.color = Color.red;
                    Handles.SphereHandleCap (i, pos, Quaternion.identity, 0.5f, EventType.Repaint);
			}
			break;
		case FormationManager.Formations.Circle:
